...
public class LoginJob extends SimpleJsonRemoteJob {
    ...
    static public class FinishAuthJob extends SimpleJsonRemoteJob {
        ...
        public String getJsonHeader(JsonRPC jsonRPC) throws IOException {
            try {
                return jsonRPC.call(METHOD,
                        ...
                        new JsonRPC.Argument(ENC_GY, Base64.encodeToString(verifier.getEncGy(), Base64.DEFAULT)),
                        new JsonRPC.Argument(VERIFICATION_TOKEN_VERIFIER,
                                Base64.encodeToString(verifier.getAuthToken(), Base64.DEFAULT)));
        ...
        @Override
        protected Result handleJson(JSONObject returnValue, InputStream binaryData) throws JSONException {
            byte[] proverToken = Base64.decode(returnValue.getString(VERIFICATION_TOKEN_PROVER), Base64.DEFAULT);
    ...
    protected Result handleJson(JSONObject returnValue, InputStream binaryData) {
        try {
            ...
            // prover.
            byte[] encGX = Base64.decode(returnValue.getString(ENC_GX), Base64.DEFAULT);
